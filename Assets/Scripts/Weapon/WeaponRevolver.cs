...
public class WeaponRevolver : WeaponBase
{
    ...
    {
        if (type == 0 && !isAttack && !isReload) OnAttack();
    }
    ...
    public override void StartReload()
    {
        ...
        if (isReload || weaponSetting.currentMagazine < 1) return;
    ...
    public void OnAttack()
    {
        ...
        {
            // 뛰고있을 때는 공격할 수 없다
            if (animator.MoveSpeed > 0.3f) return;
            
            // 공격주기가 되어야 공격할 수 있도록 하기 위해 현재 시간 저장
            lastAttackTime = Time.time;
            
            // 탄 수가 없으면 공격 불가능
            if (weaponSetting.currentAmmo < 1) return;
